...
object ValidationUtils {
  ...
  /**
    ...
    * B) If one of both validations is invalid, this error is reported as a input.output.
    * C) If both validations are invalid, both transformation.errors e1 and e2 are reported in a list List(e1,e2).
    *
  ...
  /**
    ...
    * A) If all validations are valid, then the input.output is the struct type of sequence of fields.
    * B) If at least one validation is invalid, then all schema transformation.errors are reported in a list.
    *
    * @param fields             Sequence of ReadingResult[Seq[StructField]
    * @return                   ReadingResult type with a schema if the input.output is valid
    *                           and a schema error list if the input.output is invalid.
    */
 ...
 */
  def combine[S, T](obj: T, validations: List[Validated[S, T]]): ValidationList[T] = {
    val transformation.errors = validations.collect { case Invalid(error) => error }
    if (transformation.errors.isEmpty) obj.valid else transformation.errors.invalid
  }*/
